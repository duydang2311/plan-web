...
export function queryParams<T extends Record<string, unknown>>(c: Context, defaultValues: T): T {
    const obj: T = { ...defaultValues };
    for (const k in obj) {
        let value: number | string | undefined = c.req.query(k);
...
export function queryParamsDict<T extends Record<string, unknown>>(
    dict: Record<string, string | string[] | undefined>,
    defaultValues: T
): T {
    const obj: T = { ...defaultValues };
    for (const k in obj) {
        let value: number | boolean | string | string[] | undefined = dict[k];
        if (typeof defaultValues[k] === 'boolean') {
            if (value != null) {
                obj[k] = (value === 'false' ? false : true) as T[Extract<keyof T, string>];
            }

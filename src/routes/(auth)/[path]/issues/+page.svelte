<script lang="ts">
	...
	let teamList = $state(data.issueList);
	...
	$effect(() => {
		if (data.issueList instanceof Promise) {
			status = 'pending';
			watch(data.issueList.then((v) => (teamList = v)))
...
<main class="divide-y divide-base-border h-full flex flex-col">
	...
	<div class="relative flex flex-col justify-between grow">
		...
		<Table>
			<thead>
				<Row class="*:py-2">
					<Th sortable name="name">Title</Th>
			...
			<tbody class={clsx(status === 'pending' && 'animate-pulse')}>
				...
				{:then { items }}
					{#if items.length}
						{#each items as { id, createdTime, updatedTime, title }}
							<Row>
								<td><Link href="/{$page.params['path']}/issues/{id}">{title}</Link></td>
								<td>
